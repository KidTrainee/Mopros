...
import jp.ienter.expandableview.R;
...
public class ExpandableLinearLayout extends LinearLayout implements IExpandableLayout {
    ...
    /**
     * The close position is width from left of layout_expandable_view if orientation is horizontal.
     * The close position is height from top of layout_expandable_view if orientation is vertical.
     */
    ...
    private boolean shouldExpandWhenCreate;

    private boolean shouldRecalculateSize;
        ...
        if (!isCalculatedSize) {
            ...
            if (childCount > 0) {
                ...
                for (int i = 0; i < childCount; i++) {
                    ...
                    Log.d(TAG, "onMeasure: sumSize[" + i + "] = \t" + tag
                            + "\n sumSize = " + sumSize
                            + "\n view: width = " + view.getMeasuredWidth()
                            + "\t height = " + view.getMeasuredHeight());

                }
                layoutSize = childSizeList.get(childCount - 1) +
                        (isVertical() ? getPaddingTop() + getPaddingBottom() : getPaddingLeft() + getPaddingRight());
                Log.i(TAG, "onMeasure: layoutSize = " + layoutSize);
                isCalculatedSize = true;

                if (shouldExpandWhenCreate) {
    ...
    // TODO: 3/31/2018 try to calculate size of all views inside view group
    public void recursiveLoopChildren(ViewGroup parent, int sumSize) {
        ...
        if (childCount > 0) {
            for (int i = 0; i < parent.getChildCount(); i++) {
                ...
                if (child instanceof ViewGroup) {
                    recursiveLoopChildren((ViewGroup) child, sumSize);
                } else {
                    // DO SOMETHING WITH VIEW
                    try {

                        addUpTotalSize(child, (LinearLayout.LayoutParams) childParams, sumSize);

                    } catch (ClassCastException e) {

                        addUpTotalSize(child, (FrameLayout.LayoutParams) childParams, sumSize);
    ...
    private void addUpTotalSize(View view, FrameLayout.LayoutParams params, int sumSize) {
        childSizeList.add(
                (isVertical()
                        ? view.getMeasuredHeight() + params.topMargin + params.bottomMargin
                        : view.getMeasuredWidth() + params.leftMargin + params.rightMargin
    ...
    private void addUpTotalSize(View view, LinearLayout.LayoutParams params, int sumSize) {
        childSizeList.add(
                (isVertical()
                        ? view.getMeasuredHeight() + params.topMargin + params.bottomMargin
                        : view.getMeasuredWidth() + params.leftMargin + params.rightMargin
    ...
    @Override
    public void expand() {
        Log.d(TAG, "expand: " + isAnimating);
        if (isAnimating) return;
        Log.d(TAG, "expand: " + getCurrentPosition() + "\t" + layoutSize + "\t" + duration + "\t" + interpolator);
        createExpandAnimator(getCurrentPosition(), layoutSize, duration, interpolator).start();
    }
    ...
    /**
     * Initializes this layout_expandable_view.
     */
    ...
    /**
     * Gets the width from left of layout_expandable_view if orientation is horizontal.
     * Gets the height from top of layout_expandable_view if orientation is vertical.
     *
    ...
    public int getCurrentPosition() {
        Log.i(TAG, "getCurrentPosition: " + getMeasuredHeight() + "\t" + getMeasuredWidth());
    ...
    /**
     * Set true if expandable layout_expandable_view is used in recycler view.
     *
     ...
     * Expand the layout_expandable_view if {@param to} is bigger than {@param from}.
     * Collapse the layout_expandable_view if {@param from} is bigger than {@param to}.
     *
     * @param from
     * @param to
     * @param duration
    ...
    public void expandingWhenCreate() {
        shouldExpandWhenCreate = true;
    }

    public void recalculateLayoutSize(int height) {
        shouldRecalculateSize = true;
        Log.d(TAG, "recalculateLayoutSize: childCount = " + getChildCount());
